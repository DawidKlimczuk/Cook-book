    ...
    if (empty($title) || empty($ingredients) || empty($description)) {
        ...
        if (!$error) {
            try {
                ...
                // Dodawanie przepisu
                $stmt = $pdo->prepare("INSERT INTO recipes (user_id, title, ingredients, description, image_path) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$_SESSION['user_id'], $title, $ingredients, $description, $imagePath]);
