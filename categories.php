...
$categories = [
    'sniadania' => [
        'name' => 'Śniadania',
        'icon' => '🍳',
        'tags' => ['sniadanie', 'drugie_sniadanie']
    ],
    'obiady' => [
        'name' => 'Obiady',
        'icon' => '🍝',
    ...
    'przekaski' => [
        'name' => 'Przekąski',
        'icon' => '🥪',
        'tags' => ['drugie_sniadanie', 'podwieczorek']
    ]
];
...
if ($selectedCategory && isset($categories[$selectedCategory])) {
    $categoryTags = $categories[$selectedCategory]['tags'];
    $placeholders = array_map(function($i) { return ":tag$i"; }, array_keys($categoryTags));
    
    $sql .= " AND r.id IN (
        SELECT DISTINCT recipe_id FROM recipe_tags rt2
        JOIN tags t2 ON rt2.tag_id = t2.id
        WHERE t2.name IN (" . implode(',', $placeholders) . ")
    )";
    
    foreach ($categoryTags as $i => $tag) {
        $params["tag$i"] = $tag;
    }
}
...
foreach ($categories as $key => $category) {
    $countSql = "SELECT COUNT(DISTINCT r.id) 
                 ...
                 WHERE t.name IN ('" . implode("','", $category['tags']) . "')";
